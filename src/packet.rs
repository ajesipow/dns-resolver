...
pub(crate) fn parse_dns_packet(value: &[u8]) -> Result<DNSPacket, ()> {
    ...
    let answers: Vec<DNSRecord> = (0..header.num_questions)
        .map(|_| parse_record(&mut cursor))
        .collect::<Result<Vec<_>, _>>()?;
    let authorities: Vec<DNSRecord> = (0..header.num_questions)
        .map(|_| parse_record(&mut cursor))
        .collect::<Result<Vec<_>, _>>()?;
    let additionals: Vec<DNSRecord> = (0..header.num_questions)
        .map(|_| parse_record(&mut cursor))
        .collect::<Result<Vec<_>, _>>()?;
    Ok(DNSPacket {
        header,
